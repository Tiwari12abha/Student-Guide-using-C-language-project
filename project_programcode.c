...
{ClearConsoleToColors(17,15);
	...
	go(12,15);printf("Name:");go(12,35);
	go(14,15);printf("Username:");go(14,35);
	go(16,15);printf("Mobile no.:");go(16,35);
	go(18,15);printf("Password:");go(18,35);
	go(20,15);printf("Confirm password:");go(20,35);
	go(22,35);
	...
	go(14,35);gets(u.username);
	go(22,35);clreol(25);
	printf("ENTER YOUR MOBILE NO.");

	for(;;){   go(16,35); clreol(strlen(u.mobile));
	gets(u.mobile);
	if(strlen(u.mobile)<10 || strlen(u.mobile)>10)
	{go(22,35);clreol(40);
...
void screen2_1_1(){
    ...
    go(8,32);scanf("%f",&p.maths);
	if(validmarks(p.maths))
	{
		break;
	}
	else {
...
void changepassword(){char un[51],ch;
	int found=0,i=0;
	system("cls");
		fp=fopen("newuser.dat","rb+");
	...
	switch(found){
		case 1: go(12,15);printf("new password : "); fflush(stdin);go(12,35);
		while(i<50){
		ch=getch();
		if(ch==13) break;
		else if(ch==8){
	...
	case 0:	go(20,15);printf("User not found\n");system("pause");screen2();
	}
}
